...
colnames(data_table) <- header_table

# 4. Limpeza Básica (Remover linhas vazias que o ASPX costuma gerar)
df_final <- data_table %>%
  filter(!is.na(Data) & Data != "") %>%
  mutate(across(everything(), ~as.character(.))) # Mantém como texto para evitar erros de conversão iniciais

print("--- TABELA DE PEDRA (JEQUIÉ) RECONSTRUÍDA ---")
print(df_final)
